<template>
  <div class="main-container">
    ...
    <div class="content-box">
      ...
      <div class="d-flex flex-wrap border-btn-list"> 
        <div v-ripple class=" my-4" @click="$router.push({name:'CAApplyPassword'})">
          <h3>Apply for password</h3>
        </div>
        <div v-ripple class="border-btn-item my-4">
          <h3>Using iAM Smart</h3>
        </div>
        <div v-ripple class="border-btn-item my-4">
          <h3>Using Digital Certificate</h3>
        </div>
      </div>
      <div class="d-flex flex-wrap mt-7 w-100">
        <router-link class="no-dec" :to="{ name: 'CreateAccount'}"><v-btn depressed class="o-btn-action rounded-pill mt-4 mr-5" color="#413E56" dark>Back</v-btn></router-link>
      </div>
    </div>
  </div>
</template>
